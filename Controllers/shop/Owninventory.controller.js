...
exports.createOwnInventory = async (req, res) => {
  try {
    ...
    shop.OwnInventory = newInventory._id;
    await shop.save();
...
exports.getAllOwnInventory = async (req, res) => {
  try {
    const { userId } = req.user;
    const shop = await fitmentshop
      .findOne({ userId: userId })
      .populate("OwnInventory");
    if (!shop) {
      return res.status(404).json({ message: "Shop not found for this user" });
    }
    // Return as an array for compatibility, or just the object
    res.status(200).json(shop.OwnInventory ? [shop.OwnInventory] : []);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
